...
def main(
    ...
    modTime = round_time(startTime + (endTime - startTime) / 2)  # time for plotting the model

    # Filenames 
    potFile = os.path.expanduser(startTime.strftime(potFnFmt))
    velFile = os.path.expanduser(startTime.strftime(velFnFmt))
    satFile = os.path.expanduser(satFnFmt) % (startTime.day, int(sat[1:]))
    ...
    titleStr = '%s %s - %s UT' % (sat, startTime.strftime('%H:%M'), endTime.strftime('%H:%M'))
    plot_potential(potData, modTime, dmspData, modEArray, modNArray, titleStr)
...
def plot_potential(potData, time, dmspData, interpModE, interpModN, titleStr=''):
...
def round_time(tm, mins=15):
    discard = datetime.timedelta(
        minutes=tm.minute % mins, seconds=tm.second, microseconds=tm.microsecond,
    )
    tm -= discard
    if discard > datetime.timedelta(minutes=mins/2):
        tm += datetime.timedelta(minutes=mins)
    return tm
...
if __name__ == "__main__": 
    ...
    assert len(args) == 7, 'Should have 6 arguments:\n' + \
        ...
        'F16 ' + \
        '~/data/sami3/sami3_may%ia.nc ' + \
        '~/data/sami3/sami3_may%i_phi.nc ' + \
        'data/dmsp/dms_ut_201405%i_%i.002.hdf5 '
